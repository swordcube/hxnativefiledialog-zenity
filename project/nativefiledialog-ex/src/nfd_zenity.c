...
nfdchar_t* NEXT_ZENITY_DIALOG_TITLE = NULL;
...
void NFD_SetDialogTitle(const nfdchar_t* title) {
    NEXT_ZENITY_DIALOG_TITLE = (nfdchar_t*)title;
}
...
NFD_OpenDialog(const char* filterList, const nfdchar_t* defaultPath, nfdchar_t** outPath)
{
    ...
    char *nextTitle = NEXT_ZENITY_DIALOG_TITLE;
    if (nextTitle == NULL)
        nextTitle = "Open File";

    char *message = malloc (sizeof(titlePrefix) + strlen(nextTitle) + 1);
    if (message == NULL)
        message = "";

    strcpy(message, titlePrefix);
    strcat(message, nextTitle);
    
    command[2] = strdup(message);
    free(message);
